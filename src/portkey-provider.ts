...
export type PortkeyCompletionSettings = Omit<CompletionsBodyNonStreaming, 'prompt'>;
export type PortkeyChatCompletionSettings = Omit<ChatCompletionsBodyNonStreaming, 'messages'>;
...
export function createPortkey(
  ...
  (modelId: string) => new PortkeyCompletionLanguageModel(modelId, portkeyProvider, {});

  const createCompletionModel = (modelId: string, settings?: PortkeyCompletionSettings) => {
    return new PortkeyCompletionLanguageModel(modelId, portkeyProvider, settings);
  }
  const createChatModel = (modelId: string, settings?: PortkeyChatCompletionSettings) => {
